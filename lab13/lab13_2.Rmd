...
bbox_shark <- make_bbox(long_shark, lat_shark, f = 0.03)
```

2. Load a map from `stamen` in a terrain style projection and display the map.
```{r}
map_shark <- get_stadiamap(bbox_shark, maptype = "stamen_terrain", zoom=7)
```
...
```{r}
lat_fatal <- c(32.85, 39.58)
long_fatal <- c(-123.8, -117.3)
bbox_fatal <- make_bbox(long_fatal, lat_fatal, f = 0.03)
```

```{r}
map_fatal <- get_stadiamap(bbox_fatal, maptype = "stamen_terrain", zoom=7)
```
