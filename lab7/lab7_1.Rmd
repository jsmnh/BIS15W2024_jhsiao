...
```{r}
msleep %>% 
  summarize(mean_sleep_all=min(sleep_total),
            min_sleep_all=max(sleep_total),
            max_sleep_all=mean(sleep_total),
            total=n())
```
...
Even if you have a whole bunch of value, but if you have a NA for one of them, the summarize function comes out as NA. 
```{r}
msleep %>% 
...
msleep %>% 
  ...
  select(order, genus, brainwt) #Data says no one has measured brain weight of whales. 
```
