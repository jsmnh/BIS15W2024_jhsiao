...
```{r}
fish <- readr::read_csv("data/Gaeta_etal_CLC_data.csv") ##readr is just being explicit on the package used
```
...
```{r}
select(fish, "lakeid", "scalelength") #function is select, name of dataframe(not in quote bc not variables), then specify(variables)
```
```{r}
names(fish)
```
...
```{r}
select(fish, fish_id:length) #gives the columns in between
```
...
```{r}
select(fish, matches("a.+er")) #handy bit of code for the project, look at variables of interest
```
...
Load the data. (It's alr a dataframe)
```{r}
mammals <- read.csv("data/mammal_lifehistories_v2.csv")
```

```{r}
getwd() #have to specify its in the data function
```
...
```{r}
anyNA(mammals) #it says no NA
```

```{r}
summary(mammals) #It shows min of negative number, someone put -999 instead of NA
```
