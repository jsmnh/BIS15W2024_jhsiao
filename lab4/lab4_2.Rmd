...
```{r}
filter(fish, lakeid == "AL") # use ==, use "", using select is bad practice
```
...
```{r}
filter(fish, length %in% c(167, 175)) #filter within variable length
```
...
```{r}
filter(fish, near(radii_length_mm, 2, tol = 0.2)) #look for radii length near 2, but within 0.2 tolereance
```
