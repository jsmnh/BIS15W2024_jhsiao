...
```{r}
fish$lakeid <- as.factor(fish$lakeid) #factor is when theres only two possible values to go with it
levels(fish$lakeid) #asking for levels gives all possible values 
```

Now we can pull out any state of interest.  
```{r}
filter(fish, lakeid=="LJ") #pull out all observations for lake LJ
```
...
```{r}
filter(fish, lakeid == "AL" & length > 350) # Filter fish data, lake id is AL, and length greater than 350
```

Notice that the `|` operator generates a different result.
```{r}
filter(fish, lakeid == "AL" | length > 350) #vertical line stands for OR, wil have multiple lake id
```
...
#filter(mammals_new, genus == "Lophocebus" | genus == "Erythrocebus" | genus == "Macaca")
```
Longer way:
```{r}
mammals_primates <- filter(mammals_new, order == "Primates")
mammals_final <- filter(mammals_primates, genus %in% c("Lophocebus", "Erythrocebus", "Macaca"))
```
