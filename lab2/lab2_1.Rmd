...
NA means is unable to calculate a mean as there is a missing data. It means that even if you have a lot of values, it wouldn't be calculate a mean if there is an NA. 

4. Recalculate the mean using the following code chunk. Why is the useful?  
```{r}
mean(new_vector, na.rm=T) #na.rm removes the NA values in the vector. T = true, so it would be removed. 
```
